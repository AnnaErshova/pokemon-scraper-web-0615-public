...
describe "Pokemon" do
  ...
  describe "has caught all 151 from scraping" do
    it "has all 151 pokemon" do
      expect(@db.execute("SELECT COUNT(name) FROM pokemon;").flatten.first).to eq(151)
    end
  ...
  describe "saving more pokemon to the database" do
    ...
    it "knows the pokemon count increases" do
      expect(@db.execute("SELECT COUNT(name) FROM pokemon;").flatten.first).to eq(152)
    end

    it "has Togepi as the last pokemon" do
      expect(@db.execute("SELECT name FROM pokemon WHERE id = 152;").flatten.first).to eq("Togepi")
    end
  end
  ...
  describe "BONUS" do
    ...
    it "alters Pikachu's hp to 59" do
      pikachu.alter_hp(59) # updates code first
